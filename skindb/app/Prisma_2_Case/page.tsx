...
async function CollectionPage(){
    ...
    return(
        <div>
            <Navbar></Navbar>
            <div className=" flex text-center text-black justify-center items-center bg-[#FFFBDE] w-full h-[50px]"><h1>Prisma 2 Case</h1></div>
            <div className={`grid grid-cols-4 items-center justify-center m-3 p-4`}>
                {data.map((skin)=>
                {
                    ...
                    return(
                    <SkinTile imageUrl={`/Prisma_2_Case/${skin.name}.jpeg`} name={skin.name} rarity={skin.rarity} order={order} color={color} key={skin.id}/>
                )})}
            </div>
        </div>
    )
